...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _query_hf_dataset(self, query_indices) -> dict:
        return {
            key: torch.stack([x for x in self.hf_dataset.select(q_idx)[key]])
            for key, q_idx in query_indices.items()
            if key not in self.meta.video_keys
        }

    def _query_videos(self, query_timestamps, ep_idx: int) -> dict:
    ...
    def _add_padding_keys(self, item: dict, padding) -> dict:
    ...
    def get_one_episode(self, idx) -> dict:
    ...
    def create_episode_buffer(self, episode_index = None) -> dict:
    ...
    def add_frame(self, frame: dict) -> None:
        ...
        for key in frame:
            ...
            if self.features[key]["dtype"] not in ["image", "video"]:
                ...
                #print(frame[key], frame[key].shape, frame[key].dtype, img_path)
                self._save_image(frame[key], img_path)
                self.episode_buffer[key].append(str(img_path))

        self.episode_buffer["size"] += 1

    def save_episode(self, task: str, encode_videos: bool = True, episode_data = None) -> None:
        """
